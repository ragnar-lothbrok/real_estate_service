...
import com.assignment.realestate.repo.LoginRepository;
...
public class LoginServiceImpl implements LoginService {
    ...
    public String login(String username, String password) {
        ...
        RealEstateUser realEstateUser = userRepository.findByEmailId(username);
        Optional<UserCredential> savedUserCredential = loginRepository.findById(realEstateUser.getUserId());
        if(savedUserCredential.isPresent() && savedUserCredential.get().getPassword().equals(password)) {
            String jwtToken = Jwts.builder()
                    .claim("email", username)
                    .claim("userId", realEstateUser.getUserId())
                    .setId(UUID.randomUUID().toString())
                    .setIssuedAt(Date.from(Instant.now()))
                    .setExpiration(Date.from(Instant.now().plus(24l, ChronoUnit.DAYS)))
                    .signWith(privateKey)
                    .compact();
            return jwtToken;
        } else if(savedUserCredential.isPresent() && !savedUserCredential.get().getPassword().equals(password)) {
    ...
    public Integer getUserIdFromToken(String token) {
        ...
        return jwt.getBody().get("userId", Integer.class);
    }
    ...
    private static PrivateKey getPrivateKey() throws NoSuchAlgorithmException, InvalidKeySpecException, InvalidKeySpecException, NoSuchAlgorithmException {
